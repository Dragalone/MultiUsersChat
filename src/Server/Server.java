            ...
            while (true) {
                try {
                    ...
                    if (responseMessage.getTypeMessage() == MessageType.USER_NAME && userName != null && !userName.isEmpty() && !model.getAllUsersMultiChat().containsKey(userName)) {
                        ...
                        //отправляем клиенту множестство имен всех уже подключившихся пользователей
        ...
        private void messagingBetweenUsers(Connection connection, String userName) {
            while (true) {
                try {
                    ...
                    if (message.getTypeMessage() == MessageType.CAPSLOCK_TEXT) {
                        String textMessage = String.format("%s: %s\n", userName, StringsController.caps(message.getTextMessage()));
                        sendMessageAllUsers(new Message(MessageType.TEXT_MESSAGE, textMessage));
                    }
                    if (message.getTypeMessage() == MessageType.TRANSLATE_TEXT) {
                        String textMessage = String.format("%s: %s\n", userName, StringsController.translate(message.getTextMessage()));
                        sendMessageAllUsers(new Message(MessageType.TEXT_MESSAGE, textMessage));
                    }
                    if (message.getTypeMessage() == MessageType.TRANSLATE_AND_CAPS) {
                        String textMessage = String.format("%s: %s\n", userName, StringsController.caps(StringsController.translate(message.getTextMessage())));
                        sendMessageAllUsers(new Message(MessageType.TEXT_MESSAGE, textMessage));
                    }
